...
\end{align*}
Within an isolation level, we may also choose to include restrictions on real time order, which is reasonalbe and we expect many implementations to obey this e.g. two non-concurrent transactions executed in some real time order should be expected to execute by the database in that order.
