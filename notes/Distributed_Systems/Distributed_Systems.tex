...
In order to achieve ACID guarantess of classic database systems in distributed setting, we typically rely on some kind of distributed transaction commit protocol (e.g. two-phase commit). The classic, gold standard approach for doing this in a distributed setting is to use some variant of two-phase commit protocol \cite{1986concurrency}. That is, we essentially ask multiple, separate partitions of the database data to \textit{prepare} to commit their transactions (e.g. taking locks or marking the appropriate records) before we go ahead anc \textit{commit}, if all partitions agreed.
