...
Intuitively, we can view this as determining whether two actions, $A_1$ and $A_2$, ``interfere" with each other. If we think about these actions as more or less guarded commands, this interference could mean that writes by one actino can "affect" the other. 
...
\begin{itemize}
    \item \textbf{Enabledness}: From any state, 
    \begin{itemize}
        \item $Enabled(A_2) \wedge A_1 \Rightarrow Enabled(A_2)$ \quad ($A_1$ cannot disable $A_2$)
        \item $\neg Enabled(A_2) \wedge A_1 \Rightarrow \neg Enabled(A_2)$ \quad ($A_2$ cannot enable $A_1$)
    \end{itemize}
    \item \textbf{Commutativity}: From any state, if $Enabled(A_1) \wedge Enabled(A_2)$, if we take an $A_1$ step $s \overset{A_1}{\rightarrow} s'$, the update expressions of $A_2$ in $s'$ are the same as from $s$. In other words, the modifications to the variables updated by $A_2$ will be the same whether they are applied starting from $s$ or $s'$.
\end{itemize}
