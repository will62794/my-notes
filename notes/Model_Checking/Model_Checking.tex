...
In the case where $S$ is not a true invariant of your system (meaning your system violates $S$), but the initial states satisfy $S$, then there must be some CTI of $S$ that starts in $S \wedge Reach$ and transitions to a state in $Reach \wedge \neg S$ (i.e., reaches a bad state). There may be other CTIs of $S$ that simply start in an unreachable state within $S$ and leave $S$ as in the prior ``safe'' case. But, in the unsafe case, we can imagine the $CTI^*$ state as being one to find if we actually want to discover the bug in this system. And we know that if we found the bad state that is actually reachable from $Init$, then there must be a backwards path from that state back to $Init$. If we try to refine $S$ by eliminating each $CTI^*$ in this path, then we should eventually end up with a ``CTI'' that is an initial state. That is, we've refined $S$ to cut out all non-initial states of the buggy path. We can alternatively think of this through the lens of simple bug-finding via backwards reachability i.e., start from states in $\neg S$ and try to find paths from there that lead back to the initial states.
