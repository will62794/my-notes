...
\begin{center}
    \includegraphics[scale=0.15]{images/inductive_inv1/inductive_inv1.001.png}
\end{center}

In the case where $S$ is not a true invariant of your system, but the initial states satisfy $S$, then there must be some CTI of $S$ that starts in $S \wedge Reach$ and transitions to a state in $Reach \wedge \neg S$. That is, it transitions to a reachable state that violates the invariant. There may be other CTIs of $S$ that simply start in an unreachable state within $S$ and leave $S$ as in the prior ``safe'' case. But, in the unsafe case, we can imagine the $CTI^*$ state as being one to find if we actually want to discover the bug in this system. If we find this CTI, then we can discover the full buggy trace via backwards reachability. We can alternatively think of this through the lens of simple bug-finding via backwards reachability i.e., start from states in $\neg S$ and try to find paths from there that lead back to the initial states.

\begin{center}
    % \includegraphics[scale=0.15,page=2]{images/inductive_inv1.pdf}
    \includegraphics[scale=0.15]{images/inductive_inv1/inductive_inv1.002.png}
\end{center}
